...
SC_JUDGMENTS_PATH = BASE_DIR / "data" / "sc_judgments_text.jsonl"
EMBEDDING_MODEL_NAME = "all-MiniLM-L6-v2"
COLLECTION_NAME = "legal_kb"
LOCAL_LLM_REPO = "TheBloke/Mistral-7B-Instruct-v0.2-GGUF"
LOCAL_LLM_FILENAME = "mistral-7b-instruct-v0.2.Q4_K_M.gguf"
...
class QueryEngine:
    _instance = None # Singleton pattern to avoid re-initializing models
    _initialized = False
    ...
    def __init__(self, stdout=None):
        ...
        self.stdout = stdout if stdout else print # Use print if stdout not provided
        self.stdout("🚀 Initializing the Legal AI Engine in SINGLE-ENGINE BEAST MODE...")
        MODELS_PATH.mkdir(exist_ok=True)
        llm_path = MODELS_PATH / LOCAL_LLM_FILENAME

        if not llm_path.exists():
            self.stdout(f"Downloading LLM model to {llm_path}...")
            hf_hub_download(
            ...
            self.stdout("LLM model downloaded.")
...
You are an AI legal assistant. Answer the user's question based ONLY on the provided context.
        ...
        self.stdout("Engine ready for advisory.")
 ...
 Query: {query} </INST>"""
    ...
    def verify_claims(self, claims_with_sources: list) -> tuple[int, int, float]:
        ...
        try:
            ...
            self.stdout(f"Verification Error: {e}")
    ...
    def ask_api(self, query_text: str, chat_history: list = [], analyze_precedent=False, verify_claims=False):
        ...
        self.stdout("LLM invocation complete.")

        verified, total, risk = 0, 0, 0.0
        if verify_claims:
            self.stdout("Verifying claims...")
            verified, total, risk = self.verify_claims(llm_out.get("claims", []))
            self.stdout("Claims verified.")
...
class Command(BaseCommand):
    help = 'Asks a question to the Legal AI Query Engine.'
    ...
    def handle(self, *args, **options):
        ...
        analyze_precedent = options['analyze_precedent']
        verify_claims = options['verify_claims']

        self.stdout.write(self.style.SUCCESS(f'Querying AI with: "{query_text}"'))

        try:
            engine = QueryEngine(stdout=self.stdout.write)
            response = engine.ask_api(
                query_text,
                analyze_precedent=analyze_precedent,
                verify_claims=verify_claims
            )

            self.stdout.write(self.style.SUCCESS("\n--- AI Response ---"))
            self.stdout.write(f"Answer: {response.get('answer')}")
            self.stdout.write(f"Risk Score: {response.get('risk_score')}")
            self.stdout.write(f"Verified Claims: {response.get('verified_claims')}/{response.get('total_claims')}")
            self.stdout.write(f"Tags: {', '.join(response.get('tags', []))}")
            self.stdout.write(f"Execution Time: {response.get('execution_time')} seconds")
            if response.get('precedent_data'):
                self.stdout.write("\nPrecedent Data:")
                self.stdout.write(json.dumps(response['precedent_data'], indent=2))

        except Exception as e:
            raise CommandError(f"An error occurred while querying the AI: {e}")